...
export default function RightPanel({ field, onChange }) {
  if (!field) return <div className="w-96 p-4">Select a field to edit</div>;
...
<label className="text-sm">Label</label>
        <input value={field.label} onChange={e => set({ label: e.target.value })} className="w-full mb-2 p-2 rounded-lg" />
<label className="text-sm">Placeholder</label>
        <input value={field.placeholder || ''} onChange={e => set({ placeholder: e.target.value })} className="w-full mb-2 p-2 rounded-lg" />
...
{(field.type === 'select' || field.type === 'radio' || field.type === 'checkbox') && (
          <>
            ...
            <textarea value={(field.options || []).join('\n')} onChange={e => set({ options: e.target.value.split('\n').map(s => s.trim()).filter(Boolean) })} className="w-full p-2 rounded-lg h-28" />
          </>
        )}
<div className="mt-4">
